  ...
  if (byName) {
    ...
    if (i<input->numObjects) {
      ...
      if (obj->numBytes == mo->size) {
        used.insert(obj);
        klee_warning_once(mo, "using seed input %s[%d] for: %s (no name match)", obj->name, obj->numBytes, mo->name.c_str());
        return obj;
      }
    } 
...
void SeedInfo::patchSeed(const ExecutionState &state, ref<Expr> condition, Executor *solver) {
  std::vector< ref<Expr> > required(state.constraints.begin(), state.constraints.end());
  ...
  for (std::vector< ref<ReadExpr> >::iterator it = reads.begin(), ie = reads.end(); it != ie; ++it) {
    ReadExpr *re = it->get();
    if (ConstantExpr *CE = dyn_cast<ConstantExpr>(re->index))
      directReads.insert(std::make_pair(re->updates.root, (unsigned) CE->getZExtValue(32)));
  }
  
  for (std::set< std::pair<const Array*, unsigned> >::iterator it = directReads.begin(), ie = directReads.end(); it != ie; ++it) {
