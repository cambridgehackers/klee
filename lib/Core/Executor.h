...
private:
  ...
  void branch(ExecutionState &state, const std::vector<ref<Expr>> &conditions, std::set<ref<Expr>> *values, KInstruction *target, llvm::BasicBlock *parentBlock, std::map<llvm::BasicBlock *, ref<Expr>> *targets);
