...
void *ExternalDispatcher::resolveSymbol(const std::string &name) {
  ...
  // directly access the JIT resolution function // JIT::getPointerToNamedFunction so we emulate the important points.  
  if (str[0] == 1) // asm specifier, skipped
    ++str; 
  void *addr = sys::DynamicLibrary::SearchForAddressOfSymbol(str);
  if (addr)
    return addr; 
  // If it has an asm specifier and starts with an underscore we retry // without the underscore. I (DWD) don't know why.
  if (name[0] == 1 && str[0]=='_') { 
    ++str;
...
ExternalDispatcher::ExternalDispatcher() {
  ...
  if (!executionEngine) {
    llvm::errs() << "unable to make jit:\n";
    abort();
  } 
  // If we have a native target, initialize it to ensure it is linked in and // usable by the JIT.
...
bool ExternalDispatcher::executeCall(Function *f, Instruction *i, uint64_t *args) {
  ...
  if (it == dispatchers.end()) {
    ...
    if (dispatcher) {
      ...
      // trigger crashes instead of being caught as aborts in the external // function.
...
Function *ExternalDispatcher::createDispatcher(Function *target, Instruction *inst) {
  ...
  Function *dispatcher = Function::Create(
      ...
      GlobalVariable::ExternalLinkage, "dispatcher_" + target->getName().str(), dispatchModule); 
  ...
  for (CallSite::arg_iterator ai = cs.arg_begin(), ae = cs.arg_end();
    ...
    // functions.
    LLVM_TYPE_Q Type *argTy = (i < FTy->getNumParams() ? FTy->getParamType(i) : (*ai)->getType());
    Instruction *argI64p = GetElementPtrInst::Create( nullptr, argI64s,
            ConstantInt::get(Type::getInt32Ty(getGlobalContext()), idx), "", dBB); 
  ...
  Instruction *result = CallInst::Create( dispatchTarget, llvm::ArrayRef<Value *>(args, args + i), "", dBB);
